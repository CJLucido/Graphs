...
def dft_recursive(room, last_room, way_to):
	...
	while len(traversal_graph) != 500:
		if '?' not in traversal_graph[room.id].values(): #all rooms will have exits
			if player.current_room.id == last_room:
				...
				# if player.current_room.id in traversal_graph and '?' in traversal_graph[player.current_room.id].values():
				# 	for sec_key in random.choice(list(traversal_graph[player.current_room.id].keys())): #shouldn't this already be random because dicts are unordered
				# 		if traversal_graph[player.current_room.id][sec_key] == '?':
				# 			dft_recursive(player.current_room, room.id, sec_key)
				print("moved rando",  rando)
				print("moved to rando",  player.current_room.id)
				return 
			elif way_to in traversal_graph[room.id] and traversal_graph[room.id][way_to] == last_room:
				rando = random.choice(player.current_room.get_exits())     #['n', 's', 'e', 'w'])
				traversal_path.append(rando)
				player.travel(rando)
				check_for_counter()
			...
			for key in random.choice(list(traversal_graph[room.id].keys())): #shouldn't this already be random because dicts are unordered
				...
				else: # player.current_room.id == last_room or traversal_counter[player.current_room.id] > 3:
					continue
